import { inWorkflowContext, defineSignal, defineQuery, setHandler, condition } from '@temporalio/workflow';
...
const put = defineSignal<[string]>('put');
const get = defineQuery('get');

export async function workflow(name: string): Promise<void> {
  const queue: string[] = [];

  setHandler(put, (s: string) => {
    queue.push(s);
  });
  setHandler(get, () => queue.shift());

  await condition(() => false); // TODO: ContinueAsNew
}
